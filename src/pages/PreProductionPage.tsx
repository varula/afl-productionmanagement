...
import {
  Plus, Search, Edit2, Trash2, FileCheck, Eye, ChevronRight,
} from 'lucide-react';
import { PRE_PRODUCTION_CONFIGS, DocumentTypeConfig } from '@/lib/mis-form-configs';
...
export default function PreProductionPage() {
  ...
  const [activeDoc, setActiveDoc] = useState<DocumentTypeConfig>(PRE_PRODUCTION_CONFIGS[0]);
  ...
  // Fetch documents for the active doc type
  const { data: documents = [], isLoading } = useQuery({
    queryKey: ['mis-documents', 'pre_production', activeDoc.documentType],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('mis_documents')
        .select('*, styles(style_no, buyer), lines(line_number, floors(name))')
        .eq('section', 'pre_production')
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">20 document types — CRUD data entry</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
        ...
        <Card className="border-[1.5px]">
          ...
          <CardContent className="p-1.5 max-h-[65vh] overflow-y-auto">
            {PRE_PRODUCTION_CONFIGS.map((cfg) => (
              <button
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredDocs.map((doc: any) => {
                      // Extract first text field value for "key info"
                      const data = doc.data || {};
                        ...
                        <tr key={doc.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                          ...
                          {(activeDoc.linkTo === 'style' || activeDoc.linkTo === 'style_line') && (
                            <td className="py-2 px-3">
                              {doc.styles ? (
                                <span className="text-foreground font-medium">{doc.styles.style_no}</span>
                              ) : <span className="text-muted-foreground">—</span>}
                            </td>
                          )}
                          {(activeDoc.linkTo === 'line' || activeDoc.linkTo === 'style_line') && (
                            <td className="py-2 px-3 text-muted-foreground text-xs">
                              {doc.lines ? `L${doc.lines.line_number}` : '—'}
                            </td>
                          )}
                          <td className="py-2 px-3 text-xs text-muted-foreground max-w-[200px] truncate">{keyInfo}</td>
                          <td className="py-2 px-3 text-center">
                            <Badge variant="outline" className={`text-[10px] capitalize ${STATUS_COLORS[doc.status] || ''}`}>
                              {doc.status}
                            </Badge>
                          </td>
                          ...
                          <td className="py-2 px-3">
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                onClick={() => { setEditingRecord(doc); setFormOpen(true); }}
                              >
