...
import { ClipboardList, Search, Filter, Eye, PenLine, Plus } from 'lucide-react';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { useActiveFilter, useFactoryId } from '@/hooks/useActiveFilter';

const stageColors: Record<string, string> = {
  Cutting: 'bg-primary/15 text-primary border-primary/30', Sewing: 'bg-purple/15 text-purple border-purple/30',
  QC: 'bg-warning/15 text-warning border-warning/30', Finishing: 'bg-success/15 text-success border-success/30',
  Packing: 'bg-accent/15 text-accent border-accent/30',
};
const priorityColors: Record<string, string> = {
  high: 'bg-pink/15 text-pink border-pink/30', medium: 'bg-warning/15 text-warning border-warning/30',
...
export default function OrdersPage() {
  ...
  const { data: plans = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('production_plans').select('id, date, line_id, target_qty, lines(line_number, type), styles(style_no, buyer, smv)').eq('date', today);
  ...
  const orders = useMemo(() => {
    ...
    for (const plan of plans as any[]) {
      ...
      existing.totalTarget += plan.target_qty; existing.totalOutput += outputByPlan.get(plan.id) ?? 0;
    ...
    return Array.from(styleMap.entries()).map(([key, v], i) => {
      ...
      return { id: `AAF-${3100 + i}`, buyer: v.buyer, style: v.style, pcs: v.totalTarget, output: v.totalOutput, stage, progress: Math.min(100, progress), priority, status: progress < 80 ? 'delayed' : 'active' };
    });
  }, [plans, hourly]);
  ...
  return (
    <Tabs defaultValue="view" className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground flex items-center gap-2"><ClipboardList className="h-5 w-5 text-primary" /> Active Orders</h1>
