...
import { Users, TrendingUp, UserCheck, UserX, PenLine, Eye, Search, Wrench, Upload } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from 'recharts';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { useActiveFilter, useFactoryId } from '@/hooks/useActiveFilter';
import { toast } from 'sonner';
import WorkerSkillsDialog from '@/components/workers/WorkerSkillsDialog';
import BulkImportDialog from '@/components/workers/BulkImportDialog';
...
export default function WorkersPage() {
  ...
  const [workerType, setWorkerType] = useState('operator');
  ...
  const [skillsOperator, setSkillsOperator] = useState<any>(null);
  ...
  const { data: operators = [] } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from('operators')
        .select('id, name, employee_no, grade, is_active, line_id, joined_at, designation, salary, operations_count, expertise_level, worker_type, lines(line_number, type, floor_id, floors(name))')
        .order('employee_no');
  ...
  // Fetch skills counts for all operators
  const { data: skillsCounts = {} } = useQuery({
    queryKey: ['operator-skills-counts', factoryId],
    queryFn: async () => {
      const opIds = (operators as any[]).map(o => o.id);
      if (!opIds.length) return {};
      const { data, error } = await supabase
        .from('operator_skills')
        .select('operator_id, operation_name')
        .in('operator_id', opIds);
      if (error) throw error;
      const counts: Record<string, { count: number; ops: string[] }> = {};
      for (const row of data ?? []) {
        if (!counts[row.operator_id]) counts[row.operator_id] = { count: 0, ops: [] };
        counts[row.operator_id].count++;
        counts[row.operator_id].ops.push(row.operation_name);
      }
      return counts;
    },
    enabled: (operators as any[]).length > 0,
  });
  ...
  const addMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDesignation('Operator'); setWorkerType('operator');
      setSalary(''); setOperationsCount('');
  ...
  const operatorCount = (operators as any[]).filter(o => o.worker_type === 'operator' || !o.worker_type).length;
  const helperCount = (operators as any[]).filter(o => o.worker_type === 'helper').length;
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {totalWorkers.toLocaleString()} registered · {operatorCount} operators · {helperCount} helpers
            </p>
          </div>
        ...
        <TabsContent value="view" className="space-y-4 mt-0">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <Card className="border-[1.5px]">
              <CardHeader className="pb-2"><CardTitle className="text-[13px] font-bold">Grade Distribution</CardTitle></CardHeader>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="text-center py-2 px-2 text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Type</th>
                      ...
                      <th className="text-center py-2 px-2 text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Operations</th>
                      ...
                      <th className="text-center py-2 px-2 text-[10px] uppercase tracking-wider text-muted-foreground font-semibold w-10"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredOperators.map((op: any, i: number) => {
                      const skills = (skillsCounts as any)[op.id];
                      return (
                        <tr key={op.id} className={`border-b border-border/50 hover:bg-muted/30 ${i % 2 === 0 ? 'bg-background' : 'bg-muted/10'}`}>
                          ...
                          <td className="py-1.5 px-2 text-center">
                            <Badge variant="outline" className={`text-[9px] ${
                              op.worker_type === 'helper'
                                ? 'bg-accent/15 text-accent border-accent/30'
                                : 'bg-primary/10 text-primary border-primary/30'
                            }`}>
                              {op.worker_type === 'helper' ? 'Helper' : 'Operator'}
                            </Badge>
                          </td>
                          ...
                          <td className="py-1.5 px-2 text-center">
                            <button
                              onClick={() => setSkillsOperator(op)}
                              className="inline-flex items-center gap-1 text-xs font-medium text-primary hover:underline"
                              title={skills?.ops?.join(', ') || 'Click to manage skills'}
                            >
                              {skills?.count || op.operations_count || 0}
                              {skills?.count > 0 && (
                                <span className="text-[9px] text-muted-foreground max-w-[80px] truncate">
                                  ({skills.ops.slice(0, 2).join(', ')}{skills.count > 2 ? '...' : ''})
                                </span>
                              )}
                            </button>
                          </td>
                    ...
                    {filteredOperators.length === 0 && (
                      <tr><td colSpan={13} className="py-8 text-center text-sm text-muted-foreground">No workers found</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
      ...
      <WorkerSkillsDialog
        open={!!skillsOperator}
        onOpenChange={(open) => !open && setSkillsOperator(null)}
        operator={skillsOperator}
      />
