...
export default function PlanningModule() {
  ...
  // Fetch plans for selected date with line & style info
  const { data: plans = [] } = useQuery({
    queryKey: ['planning-module-plans', selectedDate],
    queryFn: async () => {
  ...
  // Fetch hourly output for these plans
  ...
  const { data: hourly = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('plan_id, produced_qty, operators_present, helpers_present')
        .in('plan_id', planIds);
      if (error) throw error;
      return data ?? [];
    },
  ...
  const getSummary = (deptPlans: any[]) => {
    ...
    return { totalTarget, totalOutput, totalOps, avgProgress, lineCount: deptPlans.length };
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Department-wise production plan overview
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          />
          <Button size="sm" variant="outline" onClick={() => navigate('/plans/new')} className="gap-1.5">
                ...
                <CardContent className="p-0">
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {deptPlans.map((plan: any, i: number) => {
                          ...
                          return (
                            <tr key={plan.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                              ...
                              <td className="py-2.5 px-3 text-right">
                                ...
                                {plan.planned_helpers > 0 && (
                                  <span className="text-muted-foreground text-xs ml-1">+{plan.planned_helpers}h</span>
                                )}
                              </td>
                              ...
                              <td className="py-2.5 px-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className={`text-[10px] font-bold w-8 text-right ${progressColor}`}>
                                    {plan.progress}%
                                  </span>
                                </div>
                              </td>
                        ...
                        {deptPlans.length === 0 && (
                          <tr>
                            <td colSpan={10} className="py-12 text-center text-muted-foreground text-sm">
                              No {cfg.label.toLowerCase()} plans for {selectedDate}
                            </td>
                          </tr>
                        )}
                      </tbody>
                      {deptPlans.length > 0 && (
                        <tfoot>
                          <tr className="bg-muted/40 border-t font-semibold text-xs">
                            <td className="py-2 px-3" colSpan={5}>Total ({deptPlans.length} lines)</td>
